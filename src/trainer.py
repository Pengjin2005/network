...
class BPRLoss(nn.Module):
    ...
    def forward(self, 
                ...
                neg_item_embed: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        计算BPR损失
...
class NegativeSampler:
    ...
    def sample(self, 
               ...
               user_item_matrix: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        负采样
        ...
        for i, (user_idx, pos_item_idx) in enumerate(zip(user_indices, pos_item_indices)):
            # 为每个正样本生成负样本
            for _ in range(self.num_negatives):
                ...
                if user_item_matrix is not None:
                    max_attempts = 10
                    attempts = 0
                    while (user_item_matrix[user_idx, neg_item] > 0 and 
...
class DataSplitter:
    ...
    def split(self, 
              ...
              ratings: np.ndarray) -> Tuple[Dict, Dict, Dict]:
        """
        划分数据
    ...
    def _random_split(self, 
        ...
        """随机划分"""
        # 首先划分训练集和临时集
        train_users, temp_users, train_items, temp_items, train_ratings, temp_ratings = train_test_split(
        ...
        val_users, test_users, val_items, test_items, val_ratings, test_ratings = train_test_split(
            temp_users, temp_items, temp_ratings,
    ...
    def _user_based_split(self, 
        ...
        for user in unique_users:
            ...
            if n_interactions < 3:
                ...
                # 随机打乱
                indices = np.random.RandomState(self.random_state).permutation(n_interactions)
            ...
            if self.loss_type == 'mse':
                ...
                if implicit_mask.any():
                    ...
                    if len(implicit_users) > 0 and self.negative_sampler is not None:
                        ...
                        implicit_users_tensor = torch.LongTensor(implicit_users).to(self.device)
                        implicit_items_tensor = torch.LongTensor(implicit_items).to(self.device)
                        neg_users_tensor = torch.LongTensor(neg_users).to(self.device)
                        neg_items_tensor = torch.LongTensor(neg_items).to(self.device)
    ...
    def train(self,
        ...
        for epoch in range(num_epochs):
            ...
            if patience_counter >= early_stopping_patience:
                print(f"早停于第 {epoch+1} 轮，最佳验证损失: {self.best_val_loss:.4f} (第 {best_epoch+1} 轮)")
                break
