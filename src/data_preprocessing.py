...
class BookCrossingDataProcessor:
    ...
    def _create_sample_data(self):
        ...
        self.ratings_df = pd.DataFrame(
            {"User-ID": user_ids, "ISBN": book_isbns, "Book-Rating": ratings}
        )
    ...
    def _clean_books_data(self) -> None:
        ...
        self.books_df["Year-Of-Publication"] = self.books_df[
            "Year-Of-Publication"
        ].apply(clean_year)
    ...
    def filter_data(
        self, min_user_interactions: int = 5, min_book_interactions: int = 5
    ) -> None:
        """数据过滤"""
        print(
        ...
        while True:
            ...
            self.users_df = self.users_df[
                self.users_df["User-ID"].isin(self.ratings_df["User-ID"].unique())
            ]
            self.books_df = self.books_df[
                self.books_df["ISBN"].isin(self.ratings_df["ISBN"].unique())
            ]
            ...
            print(
                f"  用户: {current_users}, 书籍: {current_books}, 评分: {current_ratings}"
            )

            # 检查是否收敛
            if (
                current_users == prev_users
                and current_books == prev_books
                and current_ratings == prev_ratings
            ):
                break

            prev_users, prev_books, prev_ratings = (
                current_users,
                current_books,
                current_ratings,
            )
    ...
    def _extract_user_features(self) -> torch.Tensor:
        ...
        # 按映射顺序排序用户
        users_sorted = (
            self.users_df.set_index("User-ID")
            .loc[[self.reverse_user_map[i] for i in range(len(self.user_id_map))]]
            .reset_index()
        )
    ...
    def _extract_book_features(self) -> torch.Tensor:
        ...
        # 按映射顺序排序书籍
        books_sorted = (
            self.books_df.set_index("ISBN")
            .loc[[self.reverse_book_map[i] for i in range(len(self.book_id_map))]]
            .reset_index()
        )
...
def preprocess_book_crossing_data(
    data_path: str = "data/books/",
    min_user_interactions: int = 5,
    min_book_interactions: int = 5,
) -> Dict:
    """
